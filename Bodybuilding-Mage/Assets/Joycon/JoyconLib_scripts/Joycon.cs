    ...
    public void Update()
{
    if (state > state_.NO_JOYCONS)
    {
        ...
        while (reports.Count > 0)
        {
            ...
            if (imu_enabled)
            {
                if (do_localize) ProcessIMU(report_buf);
                else             ExtractIMUValues(report_buf, 0);
            }
            ...
            // ✅ 安全的時間字串寫法（避免 FormatException）
            var lagDequeue  = System.DateTime.Now - rep.GetTime();   // TimeSpan
            var lagBetween  = rep.GetTime() - ts_prev;               // TimeSpan
            string lagStr1  = lagDequeue.ToString(@"hh\:mm\:ss\.fff", System.Globalization.CultureInfo.InvariantCulture);
            string lagStr2  = lagBetween.ToString(@"hh\:mm\:ss\.fff", System.Globalization.CultureInfo.InvariantCulture);

            DebugPrint(
                $"Dequeue. Queue length: {reports.Count}. " +
                $"Packet ID: {report_buf[0]:X2}. Timestamp: {report_buf[1]:X2}. " +
                $"Lag to dequeue: {lagStr1}. Lag between packets (expect 15ms): {lagStr2}",
                DebugType.THREADING
            );
        ...
        {
            if (rumble_obj.t < 0) rumble_obj.set_vals(160, 320, 0, 0);
            else                  rumble_obj.t -= Time.deltaTime;
        }
    }
}

...
public class TutorialShieldController : MonoBehaviour
{
    [Header("護盾物件")]
    ...
    [Header("時間設定")]
    public bool useLifeTimer = true;   // ★ 新增：是否用時間自動破盾
    ...
    private void Update()
    {
        ...
        if (!useLifeTimer) return;   // ★ 教學模式可直接關掉時間控制
    ...
    public void BreakShieldImmediate()
    {
        StopAllCoroutines();
        isFading = false;
        BreakShield();
